...
- Sample **model configurations**, **evaluation prompts**, and **Prompty** assets
...
After this, we need to restore our dotNet packages to build our solution, we can use ``dotnet restore``. To build our ``.dll`` from solution, use ``dotnet build``, it will report two dlls being created, the test and the main one.
 ...
 To execute our solution use ``dotnet run --project .\SummarizationAPI\SummarizationAPI.csproj``. This will run our solution, which will make available our API to test and connect from our data collection located in ``chat.json``.
